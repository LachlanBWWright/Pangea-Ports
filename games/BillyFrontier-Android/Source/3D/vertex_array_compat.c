...
static DrawCacheEntry* DC_Find(const void* vptr, const void* nptr,
    ...
    const void* idxptr, int vertexCount, GLsizei indexCount, uint8_t attribMask)
{
    for (int i = 0; i < DRAW_CACHE_SIZE; i++) {
        ...
        if (e->vptr != vptr || e->nptr != nptr || e->cptr != cptr ||
            ...
            e->attrib_mask != attribMask) continue;
...
static DrawCacheEntry* DC_Alloc(void)
{
    ...
    if (!e) e = &sDC[oldest_i];
    if (!e->ibo) { glGenBuffers(5, e->attr_vbo); glGenBuffers(1, &e->ibo); }
    e->valid = 1;
    return e;
}
...
void CompatGL_DrawElements(GLenum mode, GLsizei count, GLenum type, const void* indices)
{
    ...
    {
        entry = DC_Alloc();
        entry->vptr = gVertexArrayState.vertexPointer; entry->nptr = gVertexArrayState.normalPointer;
        entry->cptr = gVertexArrayState.colorPointer;  entry->tc0ptr = gVertexArrayState.texCoordPointers[0];
        entry->tc1ptr = gVertexArrayState.texCoordPointers[1]; entry->idxptr = indices;
        entry->vertex_count = vertexCount; entry->index_count = count;
        entry->color_type = gVertexArrayState.colorType; entry->attrib_mask = attribMask;
        entry->last_used = sDC_clock++;

        if (hasPos) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_POSITION]); glBufferData(GL_ARRAY_BUFFER, vertexCount * 3 * (GLsizeiptr)sizeof(GLfloat), gVertexArrayState.vertexPointer, GL_STATIC_DRAW); uploads++; }
        if (hasNorm) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_NORMAL]); glBufferData(GL_ARRAY_BUFFER, vertexCount * 3 * (GLsizeiptr)sizeof(GLfloat), gVertexArrayState.normalPointer, GL_STATIC_DRAW); uploads++; }
        if (hasColor) {
            glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_COLOR]);
            if (gVertexArrayState.colorType == GL_FLOAT) glBufferData(GL_ARRAY_BUFFER, vertexCount * 4 * (GLsizeiptr)sizeof(GLfloat), gVertexArrayState.colorPointer, GL_STATIC_DRAW);
            else glBufferData(GL_ARRAY_BUFFER, vertexCount * 4 * (GLsizeiptr)sizeof(GLubyte), gVertexArrayState.colorPointer, GL_STATIC_DRAW);
            uploads++;
        }
        if (hasTC0) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_TEXCOORD0]); glBufferData(GL_ARRAY_BUFFER, vertexCount * 2 * (GLsizeiptr)sizeof(GLfloat), gVertexArrayState.texCoordPointers[0], GL_STATIC_DRAW); uploads++; }
        if (hasTC1) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_TEXCOORD1]); glBufferData(GL_ARRAY_BUFFER, vertexCount * 2 * (GLsizeiptr)sizeof(GLfloat), gVertexArrayState.texCoordPointers[1], GL_STATIC_DRAW); uploads++; }
        ...
        if (type == GL_UNSIGNED_INT) {
            ...
            if (count > sIdxConvertBufCap) { int newCap = count > sIdxConvertBufCap * 2 ? count : sIdxConvertBufCap * 2; if (newCap < 256) newCap = 256; free(sIdxConvertBuf); sIdxConvertBuf = (GLuint*)malloc(newCap * sizeof(GLuint)); sIdxConvertBufCap = newCap; }
            const GLushort* src = (const GLushort*)indices;
            for (GLsizei i = 0; i < count; i++) sIdxConvertBuf[i] = (GLuint)src[i];
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * (GLsizeiptr)sizeof(GLuint), sIdxConvertBuf, GL_STATIC_DRAW);
        }
        uploads++;
    }

    if (hasPos) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_POSITION]); glVertexAttribPointer(ATTRIB_LOCATION_POSITION, 3, GL_FLOAT, GL_FALSE, 0, 0); }
    if (hasNorm) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_NORMAL]); glVertexAttribPointer(ATTRIB_LOCATION_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, 0); }
    if (hasColor) {
        glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_COLOR]);
        if (entry->color_type == GL_FLOAT) glVertexAttribPointer(ATTRIB_LOCATION_COLOR, 4, GL_FLOAT, GL_FALSE, 0, 0);
        else glVertexAttribPointer(ATTRIB_LOCATION_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, 0);
    }
    if (hasTC0) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_TEXCOORD0]); glVertexAttribPointer(ATTRIB_LOCATION_TEXCOORD0, 2, GL_FLOAT, GL_FALSE, 0, 0); }
    if (hasTC1) { glBindBuffer(GL_ARRAY_BUFFER, entry->attr_vbo[ATTRIB_LOCATION_TEXCOORD1]); glVertexAttribPointer(ATTRIB_LOCATION_TEXCOORD1, 2, GL_FLOAT, GL_FALSE, 0, 0); }
