...
void PangeaNet_ReportMatchResult(const char* json);
void PangeaNet_SendPlayerExploded(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendPlayerDeathDiveImpact(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendWeaponFired(short playerNum, short weaponType, const OGLPoint3D* where);
void PangeaNet_SendWeaponHit(short playerNum, short weaponType, int terrainHit, const OGLPoint3D* where);
void PangeaNet_SendJetpackIgnited(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendJetpackShutoff(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendShieldHit(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendWormholeEntered(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendWormholeExited(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendDustDevilCaptured(short playerNum, const OGLPoint3D* where);
void PangeaNet_SendDustDevilReleased(short playerNum, const OGLPoint3D* where);
int PangeaNet_GetRemoteLifecycleReason(void);
int PangeaNet_GetLastMatchEndReason(void);
int PangeaNet_HasMatchResult(void);
