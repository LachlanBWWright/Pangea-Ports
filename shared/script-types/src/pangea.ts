...
export interface ScriptedObjectDefinition {
  ...
  readonly onDamage?: (self: ScriptedObjectSelf, amount: number, ctx: FrameContext) => number;
...
export interface PangeaApi {
  ...
  readonly object: {
    ...
    setHealth(handle: ObjectHandle, value: number): boolean;
    delete(handle: ObjectHandle): boolean;
  };
  readonly spawn: {
    native(id: string, position: Vector3, options?: NativeSpawnOptions): ObjectHandle | undefined;
    scripted(id: string, position: Vector3, options?: ScriptedSpawnOptions): ObjectHandle | undefined;
  };
}
